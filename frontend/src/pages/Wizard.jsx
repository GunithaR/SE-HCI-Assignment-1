...
function ResultsView({ resultsData, answers, visibleQuestions, selectedCategory, onTryAgain }) {
  ...
  const compareMutation = useMutation({
    mutationFn: (variables) => catalogService.compareRecommendations(variables),
    onSuccess: (result) => {
      setComparisonData(result);
      setShowComparison(true);
    },
    onError: (e) => setCompareError(e.response?.data?.message || 'Failed to compare'),
  });
  ...
  const handleCompare = () => {
    ...
    compareMutation.mutate({ selectedProductIds: Array.from(selectedIds), recommendations: sel });
  };
    ...
    return (
      <div className="results-inner">
        <div style={{ textAlign: 'center', padding: '4rem 2rem' }}>
          ...
          <button onClick={onTryAgain} className="wizard-btn-try-again">← Try Different Options</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="results-inner">
      ...
      {selectedIds.size === 0 && !showComparison && (
        <div className="compare-banner">
          ...
          <div className="compare-banner-text">
            ...
            <p>Select 2 or more products using the checkboxes to compare them side-by-side!</p>
          </div>
        </div>
      )}
      ...
      <div className="results-page-header">
        ...
        <p className="results-subtitle">{selectedCategory} — {products.length} products ranked by our AI engine</p>
      </div>
      ...
      {selectedIds.size > 0 && !showComparison && (
        <div className="compare-toolbar">
          <div className="toolbar-info"><span className="selected-count">{selectedIds.size} product{selectedIds.size !== 1 ? 's' : ''} selected</span></div>
          <div className="toolbar-actions">
            {compareError && <span className="toolbar-error">{compareError}</span>}
            <button className="toolbar-btn primary" onClick={handleCompare} disabled={compareMutation.isPending}>
              {compareMutation.isPending ? 'Comparing...' : 'Compare Selected'}
            </button>
            <button className="toolbar-btn secondary" onClick={clearSelection} disabled={compareMutation.isPending}>Clear</button>
          </div>
        </div>
      )}
...
export default function Wizard() {
  ...
  const { data: categories = [], isLoading: loadingCats } = useQuery({
    ...
    queryFn: catalogService.getQuestionCategories,
  });

  const questionMutation = useMutation({
    mutationFn: (cat) => catalogService.getQuestions(cat),
    onSuccess: (data) => {
  ...
  const recommendMutation = useMutation({
    mutationFn: (payload) => catalogService.getHybridRecommendations(payload),
    onSuccess: (data) => setResultsData(data),
  });

  const selectCategory = (cat) => {
    setSelectedCategory(cat);
    questionMutation.mutate(cat);
  };
  ...
  const visibleQuestions = useMemo(() => {
    return questions.filter((q) => {
      ...
      if (qText.includes('moisture') || q.id === 'moisture') {
        const hasCoastal = Object.values(answers).some(val => typeof val === 'string' && val.toLowerCase() === 'coastal');
        if (!hasCoastal) return false;
      }
      if (qText.includes('budget') || q.id === 'budget') {
        const hasLowCost = Object.values(answers).some(val => typeof val === 'string' && val.toLowerCase() === 'low cost');
  ...
  const goNext = () => {
    ...
    if (allAnswered) setCurrentQuestionId(null);
    else if (currentIndex < totalSteps - 1) setCurrentQuestionId(visibleQuestions[currentIndex + 1].id);
  };

  const goBack = () => {
    if (isReviewStep) setCurrentQuestionId(visibleQuestions[visibleQuestions.length - 1].id);
    else if (currentIndex > 0) setCurrentQuestionId(visibleQuestions[currentIndex - 1].id);
    else {
  ...
  const handleSubmit = () => {
    const cleanAnswers = {};
    visibleQuestions.forEach(q => { if (answers[q.id]) cleanAnswers[q.id] = answers[q.id]; });
    recommendMutation.mutate({ category: selectedCategory, answers: cleanAnswers });
  };
