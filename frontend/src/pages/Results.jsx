...
function ResultCard({ product, rank }) {
    ...
    return (
        <div className="card p-5 fade-in-up flex gap-4">
            {/* Rank badge */}
            <div className="shrink-0 w-10 h-10 rounded-full flex items-center justify-center font-bold text-white text-sm"
                style={{ background: rank <= 3 ? 'linear-gradient(135deg,#6c63ff,#f59e0b)' : 'rgba(255,255,255,0.05)' }}>
                #{rank}
            </div>
            ...
            <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between text-xs">
                    ...
                    <div className="text-right shrink-0">
                        <div className="text-violet-300 font-bold">${Number(product.basePrice).toFixed(2)}</div>
...
export default function Results() {
    ...
    return (
        <div className="min-h-screen px-6 py-28 max-w-4xl mx-auto fade-in-up">
            {/* Header */}
            <div className="mb-10">
                ...
                <p className="text-slate-400">
                    Filtered for{' '}
                    <span className="text-violet-300 font-medium">{answers.budget}</span> budget ·{' '}
                    <span className="text-violet-300 font-medium">{answers.climate}</span> climate
                </p>
